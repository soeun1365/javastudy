...
public class WeekScheduler {
	...
	public void run() {
		while(true) {
			...
			case 0 : exit(); return;//결과값이 보이드일 경우 return;으로 끝낼 수 있다.
			//dafault : System.out.println("없는 작업입니다. 다시 입력하세요");
			}
		}
	}
	...
	public void menu() {
		...
		System.out.println("5. 전체 즈케쥴 보기");
		...
		for(int i= 0 ; i < week.length ; i++) {
			if(weekNames[i].equals(weekName) ) {
				if(week[i].getSchedule() == null || week[i].getSchedule().isEmpty()) {
					...
					System.out.println(weekName + "요일은 이미 스케쥴이 있다.");
				}
				break;	//for문 종료
			}
		}
	...
	public void deleteSchedule() {
		...
		weekName = weekName.substring(0,1);	//0 <= 추출할 문자열 <1
			//weekName = weekName.charAt(0);	-->char 타입 이기에 이것을 쓰면 오류뜸
		for(int i = 0 ; i < week.length ; i++) {
			if(weekNames[i].equals(weekName)) {
				if(week[i].getSchedule() ==null || week[i].getSchedule().isEmpty()) {
					...
					System.out.print( "["+ week[i].getSchedule() + "]");
					System.out.print("삭제할까요?(Y/N)>>> ");
					String yesNo = sc.next();	//nextLine이었으면 떠도는 엔터값이 있으면 안됨
												//nextLine 이면 제거하는 코드 필요했음
												//얘는 next()이기 때문에 필요없음
					yesNo = yesNo.substring(0,1);
					if(yesNo.equalsIgnoreCase("Y")) {
						week[i].setSchedule(null);	//초기화
		...
		for(int i = 0 ; i < week.length ; i++) {
			if(weekNames[i].equals(weekName)) {
				if(week[i].getSchedule() == null || week[i].getSchedule().isEmpty()) {
					...
					sc.nextLine();	//엔터제거
					if(yesNo.charAt(0)=='Y' || yesNo.charAt(0)== 'y') {
						System.out.print(weekName + "요일의 수정스케쥴 입력>>> ");
						String schedule = sc.nextLine();
						week[i].setSchedule(schedule);
	...
	public void serchSchedule() {
		...
		String weekName = sc.next();
		weekName = weekName.substring(0,1);
			//week.Name.charAt(0) + ""; 이렇게 charAt 한 후에 문자열로 바꿔주면 같은 역할을 하는것임
		for(int i= 0 ; i < week.length ; i++) {
			if(weekNames[i].equals(weekName)) {
				System.out.println(weekName + "요일의 스케쥴입니다.");
				week[i].info();
				return;	//serchSchedule() 메소드의 종료
			}
